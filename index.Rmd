...
keygram_lunch <- get_tidy_audio_analysis("1cuVNVugfiBag4geicsJZA") %>%
...
keygram_dinner <- get_tidy_audio_analysis("1EXrFPfVNVsyb32yapebbM") %>%
  compmus_align(sections, segments) %>%
  select(sections) %>%
  unnest(sections) %>%
  mutate(
...
For these visualizations I chose again two random tracks. This time I picked the track 5th from the bottom. For the lunch playlist this is 'Window Seat' from Erykah Badu. For the evening playlist this is 'Our House' from Madness, however this track is 8 minutes long. So for practicality I picked the 6th track. 
...
In the heatmap there are three clusters to be found. The first has higher values in A and lower in accousticness. The second cluster is harder to determine. 

# Conclusion

Wie heeft er baat bij: handvatten om de beste playlists te kunnnen maken voor momenten op de dag.
